...
// Reuse d3-geo + topojson via le même pipeline qu'avant, mais fitExtent
// est calculé sur les seules préfectures du voyage pour zoomer.
...
let _tripLibsP = null;
function loadTripLibs() {
  if (_tripLibsP) return _tripLibsP;
  _tripLibsP = new Promise((resolve, reject) => {
    const load = (src) => new Promise((res, rej) => {
      // si déjà chargé par japan-map.jsx, on profite
...
function TripMap({ projects, cities, itinerary, onPickCity, hoveredCity, setHoveredCity, animations }) {
  const [paths, setPaths] = React.useState(null);
  const [pinPos, setPinPos] = React.useState({});
    ...
    <div style={{position:'relative', width:'100%', height:'100%'}}
      ...
      <svg ref={svgRef} viewBox={`0 0 ${TMAP_W} ${TMAP_H}`} preserveAspectRatio="xMidYMid meet"
        ...
        <g transform={`translate(${TMAP_W/2 + pan.x} ${TMAP_H/2 + pan.y}) scale(${zoom}) translate(${-TMAP_W/2} ${-TMAP_H/2})`}>
          {/* clip pour empêcher Hokkaido/Kyushu de déborder */}
          <defs>
